...
export class Välit {
	...
	merkitseVälit(merkittäväVäli) {
		this.välit.forEach(väli => {
			if (
				väli.kieli == merkittäväVäli.kieli &&
	...
	// palauttaa välitiedon kielen ja välin perusteella
	annaVäliTieto(xväli) {
		let a = this.välit.find(väli => {
			return väli.kieli == xväli.kieli && väli.väli == xväli.väli
		})
		return a
	}
...
function teeVälitKielelle(kieli, aloitusNuotti, oktaavi) {
	...
	let aloitusIndeksi = kaikkiNuotit.findIndex(
		nuotti => nuotti == aloitusNuotti
	)
	let kaikkiVälit = []
